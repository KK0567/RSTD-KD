...
# -*- coding: utf-8 -*-
"""
RSTD-KD 物理层干扰仿真实验 (Reviewer 1 #5)
Physical-Layer Interference Simulation

审稿人意见: 整个研究基于离线数据集仿真验证，忽略了真实物理场景中的
实时信道衰落、环境电磁噪声、动态无人机拓扑变化等干扰因素。

实验设计 — 四个维度模拟真实部署中的物理层干扰:
  1. 信道衰落 (Channel Fading)  → 特征幅度衰减 α ∈ [1.0, 0.5]
  2. 电磁噪声 (EMI Noise)       → 加性高斯白噪声 SNR ∈ [20dB, 0dB]
  3. 拓扑丢包 (Packet Loss)     → 随机特征丢失 rate ∈ [0%, 30%]
  4. 拓扑乱序 (Packet Reorder)  → 特征级信息失真 rate ∈ [0%, 30%]

原理说明:
  - 模型在干净训练数据上训练，仅在测试集上施加干扰
  - 干扰在 StandardScaler 标准化后的特征空间中实施
  - 每个干扰水平重复 3 次，取均值和标准差
  - 信道衰落: 乘以衰减系数 α，模拟 Rayleigh/Rician 衰落导致的信号幅度衰减
  - 电磁噪声: 添加 N(0, σ²) 噪声，σ²=1/SNR_linear，模拟环境电磁干扰
  - 拓扑丢包: 随机将特征置零，模拟丢包导致的信息缺失
  - 拓扑乱序: 随机替换为同特征其他样本值，模拟包乱序导致的统计偏差
...
#  路径配置
# ================================================================
RAW_DATA_CSV = r"E:\Licaiyun\practise\4.RSTD_KD\Data\ECU-IoFT-Dataset.csv"
OUTPUT_DIR = r"E:\Licaiyun\practise\4.RSTD_KD\one review\fangzhen"
...
SEED = 42
WINDOW_SIZE = 32
N_REPEATS = 5  # 每个干扰水平重复次数

# 干扰维度梯度
FADING_ALPHAS  = [1.0, 0.7, 0.5, 0.3, 0.2, 0.1]          # 信道衰减系数 (含热噪声)
NOISE_SNR_DB   = [20.0, 15.0, 10.0, 5.0, 3.0, 0.0]       # 信噪比 (dB)
LOSS_RATES     = [0.0, 0.10, 0.20, 0.30, 0.40, 0.50, 0.70] # 丢包率 (扩展到70%)
REORDER_RATES  = [0.0, 0.10, 0.20, 0.30, 0.40, 0.50, 0.70] # 乱序率 (扩展到70%)
...
# ================================================================
#  数据构建 (复用 stability_experiments.py 的特征工程)
# ================================================================
...
def apply_channel_fading(X_scaled, alpha, rng):
    ...
    物理原理: 无线信道衰落具有频率选择性，不同频带经历不同深度的衰落。
    在特征空间中，我们将 92 个特征分为 5 个组 (timing/length/info/network/proto)，
    每组经历独立的随机衰减 (模拟频率选择性)，同时衰落导致接收端热噪声增强。

    衰减模型:
      - 每组衰减系数: group_alpha_i ~ U(max(0, 2α-1), 1)
      - 组内每个特征再叠加独立微扰: feature_alpha ~ U(0.8, 1.2) * group_alpha
      - 衰落引起的热噪声: noise_std = (1 - α) * 0.5

    这确保 LayerNorm 无法完全补偿 (因为组间衰减不同)，且噪声随衰落加深而增大。
    """
    n_samples, n_features = X_scaled.shape

    # 定义特征组 (基于特征构建顺序)
    # 组 0: 时序特征 (0-6), 组 1: 长度特征 (7-19), 组 2: 信息特征 (20-22)
    # 组 3: 网络特征 (23-28), 组 4: 协议+其他 (29+)
...
def apply_emi_noise(X_scaled, snr_db, rng):
    """电磁噪声: 添加高斯白噪声，噪声功率由 SNR(dB) 决定
    σ² = 1 / SNR_linear, 信号功率假设=1 (标准化后)"""
...
def apply_packet_loss(X_scaled, loss_rate, rng):
    """拓扑丢包: 随机将 loss_rate 比例的特征值置零
    模拟通信不稳定导致的数据包丢失、信息缺失"""
    mask = rng.random(X_scaled.shape) >= loss_rate  # True=保留, False=丢失
    return X_scaled * mask.astype(np.float32)


def apply_packet_reorder(X_scaled, reorder_rate, rng):
    """拓扑乱序: 随机将 reorder_rate 比例的特征替换为同特征其他样本的值
    模拟包到达顺序混乱导致的窗口级统计偏差"""
    n_samples, n_features = X_scaled.shape
    result = X_scaled.copy()
...
# ================================================================
#  干扰评估 (使用预缩放特征, 避免 scaler 重复调用)
# ================================================================
class PreScaledDS(Dataset):
    """直接使用预缩放 (可能已被干扰) 的特征, 不再调用 scaler。"""
...
def evaluate_with_interference(model, test_std_df, fcols, scaler, t_test,
    ...
    关键: 先做 scaler.transform 得到标准化特征, 再施加物理干扰,
    然后用 PreScaledDS 直接传入干扰后的标准化特征 (跳过 scaler 二次调用)。
    """
    # 标准化
    ...
    # 使用 PreScaledDS 避免 scaler 被重复调用
...
def main():
    ...
    print(f"  原始数据包: {len(raw_df)}")

    win_df = build_windows(raw_df, WINDOW_SIZE, WINDOW_SIZE)
    fcols = get_feature_cols(win_df)
    print(f"  窗口数: {len(win_df)}, 特征数: {len(fcols)}")

    # ============================================================
    #  [2/7] 数据划分 & 标准化
    # ============================================================
    print("\n[2/7] 数据划分 (seed=42, Random Stratified)...")
    ...
    print(f"  基线: Acc={baseline['accuracy']:.4f}, MacroF1={baseline['macro_f1']:.4f}, "
    ...
    for dim_key, dim_name, levels, col_name in [
        ...
        for l in levels:
            if l == levels[0] and l in [1.0, 20.0, 0.0]: continue  # 跳过与 baseline 重复的
            r = all_results[dim_key][l]
            summary_rows.append({
