...
# -*- coding: utf-8 -*-
"""
RSTD-KD 组件消融实验 (5-Fold Cross-Validation)
Component Ablation Study for RSTD-KD with 5-Fold CV

审稿人回复：
  Reviewer 1 #3: 补充窗口滑动长度、Platt校准、任务成本权重配置的消融
  Reviewer 2 #6: 窗口大小 w=32 敏感性分析

消融变体 (每个 fold 的 test set 上报告, 最终取 5-fold 均值±标准差):
  1. Full RSTD-KD          — 完整方法
  2. w/o Knowledge Dist.   — 去掉知识蒸馏 (KD loss 权重置零)
  3. w/o Platt Calibration — 去掉 Platt 概率校准 (使用原始 softmax 概率)
  4. w/o Cost-aware Thresh  — 去掉验证集阈值搜索 (使用固定默认阈值 0.5)
  5. w=16 / w=64           — 不同窗口大小

指标: Accuracy, Balanced Accuracy, Macro-F1, High-state Recall,
      Brier Score, ECE, NLL

使用方法:
  直接运行本文件即可 (python ablation_study_rstd_kd.py)
  所有输出保存到 OUTPUT_DIR 目录
"""
...
OUTPUT_DIR = r"E:\Licaiyun\practise\4.RSTD_KD\one review\xiaorong"
...
def build_features_one_window(sub: pd.DataFrame, window_id: int) -> Dict:
    ...
    if dominant not in ATTACK_TYPE_MAP:
        raise ValueError(f"未知攻击类型: {dominant}")
...
def prepare_cv_folds(raw_df, window_size, n_folds=N_FOLDS):
    """构建窗口数据并生成 n_folds 折分层划分。返回 (folds, feature_cols)。
    folds 是 list of (train_df, val_df, test_df)。
    """
    win_df = build_windows_df(raw_df, window_size=window_size, step_size=window_size)
    feature_cols = _get_feature_cols(win_df)

    # 用 y_attack_type 做分层，保证每折的攻击类型分布一致
    ...
    for fold_i, (train_val_idx, test_idx) in enumerate(fold_indices):
        # 从 train_val 中再分 val: 取最后一折作为 val
        # 使用 n_folds-1 折做 train, 1 折做 val
        # 但这里已经有 n_folds 折了，test 占 1 折，剩下 n_folds-1 折里再分
        # 简化：train_val 有 n_folds-1 份，用最后一份做 val
...
class KDDataset(Dataset):
    def __init__(self, df, feature_cols, scaler, teacher_df):
        ...
        if miss > 0:
            raise RuntimeError(f"Teacher 对齐失败，缺失 {int(miss)} 行")
...
def run_single_fold(train_df, val_df, test_df, feature_cols, device, use_kd=True):
    """对一个 fold 运行完整 pipeline，返回该 fold 的所有变体指标。"""
    # Teacher
    ...
    if use_kd:
        ...
        model_nokd = model_full  # 如果已经要求不用 KD，则复用
...
def main():
    ...
    print(f"设备: {device}  |  Folds: {N_FOLDS}  |  Windows: {WINDOW_SIZES}")
    print(f"输出目录: {out_dir}")

    raw_df = pd.read_csv(RAW_DATA_CSV)
    raw_df["_dt"] = parse_time(raw_df["Time"])
    raw_df = raw_df.reset_index(drop=True)
    print(f"原始数据包数: {len(raw_df)}")

    # 收集所有 fold 的结果: {window_size: {variant_key: [fold1_metrics, fold2_metrics, ...]}}
    all_fold_results = {}
    ...
    for ws in WINDOW_SIZES:
        ...
        print(f"  窗口总数: {sum(len(t) for t, _, _ in folds)}")
    ...
    # --- 可靠性曲线 (使用最后一 fold 的 w=32 数据) ---
    # 为简化，在最后一个 fold 上绘制
    print("\n生成可靠性曲线 (最后一个 fold, w=32)...")
    last_fold_data = all_fold_results[MAIN_WINDOW]
    # 收集所有 fold 的预测概率用于绘图 (取最后一个成功的 fold)
    # 这里简化处理：如果中间文件存在就使用
    ...
    print(f"{'=' * 60}")
    print(f"输出文件:")
    print(f"  table1_component_ablation_w32_cv.csv  — 组件消融表 (w=32, mean±std)")
    print(f"  table2_window_sensitivity_cv.csv      — 窗口敏感性表 (mean±std)")
    print(f"  table3_full_ablation_cv.csv           — 完整消融表 (mean±std)")
    print(f"  cv_fold_details_w*.csv                — 每折详细结果")
    print(f"  ablation_cv_summary.json              — 完整 JSON")
