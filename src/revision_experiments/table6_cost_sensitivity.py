...
审稿人回复 (Reviewer 1 #4):
  需要连续变化的 FP/FN 成本比例实验，说明阈值如何随任务成本变化而动态漂移。

实验设计:
  Cost Ratios: C_FP:C_FN = 5:1, 2:1, 1:1, 1:2, 1:5
  对每个 cost ratio，扫描阈值并找到最优值。
  报告: optimal threshold, FPR, FNR, expected cost

  同时分析:
  (1) 二分类攻击检测层 (binary tau_attack)
  (2) 三级风险决策层 (tau_attack + tau_high)

使用方法:
  直接运行 (python fpfn_cost_sensitivity.py)
  所有输出保存到 OUTPUT_DIR
"""
...
OUTPUT_DIR = r"E:\Licaiyun\practise\4.RSTD_KD\one review\FPFN_cost"

# ================================================================
#  全局超参数 (与消融实验一致)
# ================================================================
SEED = 42
WINDOW_SIZE = 32  # 论文主窗口
...
def binary_cost_scan(y_true, y_prob, c_fp, c_fn, thresholds=None):
    """对二分类概率扫描阈值，计算每个阈值的 FPR/FNR/Expected Cost。"""
...
def risk_cost_scan(y_risk, bin_prob, risk_prob, c_fn_medium, c_fn_high, c_fp,
                    tau_grid=None):
    """对风险决策的 tau_attack 扫描（固定 tau_high 为其最优值），
    计算加权误分类代价。"""
    if tau_grid is None:
    ...
    for ta in tau_grid:
        ...
        # 代价矩阵: 行=真实, 列=预测
        # 真实 high 预测为 normal → 最严重 (c_fn_high)
        # 真实 normal 预测为 high → 误报 (c_fp)
        # 真实 medium 预测为 normal → 漏报 (c_fn_medium)
        fn_high = cm[2, 0] + cm[2, 1]  # 真实 high 被判为 0 或 1
        fn_med = cm[1, 0]  # 真实 medium 被判为 0
        fp = cm[0, 1] + cm[0, 2]  # 真实 normal 被判为 1 或 2
...
def plot_cost_curves(scan_results, out_dir):
    ...
    # 左图: Expected Cost vs Threshold (不同 cost ratio)
    ax = axes[0]
    ...
    # 中图: Optimal Threshold vs Cost Ratio
    ax2 = axes[1]
    ...
    # 右图: FPR vs FNR at optimal threshold
    ax3 = axes[2]
    fprs = []; fnrs = []
    labels = []
    for (cfp, cfn), df in scan_results.items():
...
def main():
    ...
    # --- 2. 训练 Teacher + Student ---
    print("\n[2/4] 训练 Teacher + Student (Full RSTD-KD)...")
    ...
    print(f"  训练完成. Test samples: {len(y_bin_test)}")

    # --- 3. 成本敏感性分析 ---
    print("\n[3/4] 运行 FP/FN Cost-Ratio Sensitivity Analysis...")
    scan_results = {}
    summary_rows = []
    ...
    # --- 风险级成本分析 ---
    print("\n  Risk-Level Cost Analysis (三级风险)...")
    risk_rows = []
    # 风险级成本: C_FN_high : C_FN_medium : C_FP
    risk_configs = [
        ("5:1:1", 1, 5, 1),    # 高风险漏报代价极高
        ("2:1:1", 1, 2, 1),
        ("1:1:1", 1, 1, 1),    # 对称
        ("1:1:2", 2, 1, 1),
        ("1:1:5", 5, 1, 1),    # 误报代价极高
    ...
    print(f"  1. cost_ratio_summary.csv          — 二分类成本敏感性汇总表 (核心表)")
    print(f"  2. risk_level_cost_summary.csv     — 三级风险成本分析表")
    print(f"  3. cost_scan_*.csv                 — 每个 cost ratio 的逐阈值扫描明细")
    print(f"  4. cost_sensitivity_curves.png/pdf — 成本敏感性曲线图")
    print(f"  5. student_test_predictions.csv    — 学生模型测试集预测概率")
    print(f"  6. cost_analysis_summary.json      — 完整 JSON 汇总")
