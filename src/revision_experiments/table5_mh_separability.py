...
# -*- coding: utf-8 -*-
"""
RSTD-KD 蒸馏损失对 Medium/High 可分性影响分析 (5-Fold CV)
Distillation Loss Impact on Medium/High Separability

审稿人回复 (Reviewer 2 #7):
  蒸馏损失把教师模型的二分类攻击概率与学生模型的 Medium+High 聚合风险概率对齐，
  会不会削弱 Medium 和 High 两类之间的可分性？

实验设计:
  1. Per-class precision / recall / F1 (Normal, Medium, High)
  2. Medium/High 混淆矩阵
  3. 不同蒸馏权重下的 Medium/High 分类表现 (5-fold CV mean±std):
     - Full KD (默认权重)
     - No KD (蒸馏权重全部为零)
     - KD x0.5 (蒸馏权重减半)
     - KD x2.0 (蒸馏权重加倍)
     - Risk KD Only (仅风险级蒸馏)
     - Bin+Attack KD Only (仅二分类+攻击类型蒸馏, 无风险级蒸馏)
  4. 概率级分析: risk probability 分布, decision margin
  5. t-SNE 特征可视化 (backbone 最后一层)

核心解释:
  蒸馏损失约束的是攻击级别概率 (Medium+High 的总攻击概率)；
  Medium 和 High 的区分仍然由多分类风险状态监督损失学习。
  因此，蒸馏并不是把 Medium 和 High 合并，而是提供攻击级别的正则化。

使用方法:
  直接运行 (python kd_medium_high_separability.py)
  所有输出保存到 OUTPUT_DIR
"""
...
OUTPUT_DIR = r"E:\Licaiyun\practise\4.RSTD_KD\one review\zhengliu"
...
def prepare_cv_folds(raw_df, ws=WINDOW_SIZE):
    """构建窗口并生成 5-fold CV 划分。返回 [(train_df, val_df, test_df, fcols), ...]。"""
...
def add_label_noise(train_df, noise_rate, seed):
    """对 Medium/High 风险标签添加随机翻转噪声。

    仅翻转 Medium(1) ↔ High(2)，不动 Normal(0)。
    """
    if noise_rate == 0:
...
def run_noise_experiment(folds, device):
    """标签噪声鲁棒性实验: Full KD vs No KD, 不同噪声率。"""
...
def run_fold_analysis(train_df, val_df, test_df, fcols, device):
    ...
    fold_results = {}
    fold_features = {}
    fold_probs = {}  # 保存 risk probabilities 用于概率分析
    ...
    for cname, cfg in KD_CONFIGS.items():
        ...
        # 提取特征 (仅对 Fold 1 的 Full KD 和 No KD)
...
def main():
    ...
    print(f"设备: {device}  |  Folds: {N_FOLDS}  |  输出: {out_dir}")
    ...
    print(f"  总窗口数: {sum(len(t) for t, _, _, _ in folds)}")
    for fi, (tr, va, te, fc) in enumerate(folds):
        print(f"  Fold {fi+1}: Train={len(tr)}, Val={len(va)}, Test={len(te)}, "
              f"Test风险=[N={int((te['y_risk']==0).sum())}, M={int((te['y_risk']==1).sum())}, H={int((te['y_risk']==2).sum())}]")
    ...
    print("  -> medium_high_analysis.png/pdf 已保存")

    if fold1_features is not None:
        print("  -> 生成 t-SNE 图...")
        plot_tsne(fold1_features, fold1_labels, out_dir)
        print("  -> tsne_features.png/pdf 已保存")

    # ================================================================
    #  标签噪声鲁棒性实验
    # ================================================================
    print("\n[5/5] 标签噪声鲁棒性实验 (Full KD vs No KD)...")
    print(f"  噪声率: {NOISE_RATES}")
    ...
    print("  -> noise_robustness.png/pdf 已保存")
    ...
    print(f"  1. per_class_metrics_cv.csv        — 每类 P/R/F1 (5-fold CV mean±std)")
    print(f"  2. medium_high_separability_cv.csv — Medium/High 可分性指标 (5-fold CV)")
    print(f"  3. probability_analysis_cv.csv     — 概率级分析 (margin/entropy)")
    print(f"  4. confusion_matrices_fold1.txt    — Fold 1 的 3x3 混淆矩阵")
    print(f"  5. medium_high_analysis.png/pdf    — 主对比图 (9 面板)")
    print(f"  6. tsne_features.png/pdf           — t-SNE 特征可视化")
    print(f"  7. noise_robustness_results.csv    — 标签噪声鲁棒性实验结果")
    print(f"  8. noise_robustness.png/pdf        — 噪声鲁棒性可视化")
    print(f"  9. analysis_summary.json           — 完整 JSON 汇总")
