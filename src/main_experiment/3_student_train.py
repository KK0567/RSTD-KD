...
"""
ECU-IoFT student cascade, compatible with attack-stratified / run-level / time-level splits.

Key changes against the original version:
1) Reads fixed teacher attack probability columns generated by the compatible teacher script.
2) Uses fixed-label metrics for risk grading: labels=[0,1,2]. This prevents Macro-F1 inflation
   when strict run/time splits miss one class in val/test.
3) Keeps the 3-neuron attack head, but safely handles missing attack types in the training split.
4) Adds high-risk recall and split distribution to the output summary.
"""
...
# student attack head internal class ids 0/1/2 correspond to original attack ids 1/2/3
...
def sanitize_feature_cols(df: pd.DataFrame, feature_cols: List[str]) -> List[str]:
    ...
    for c in feature_cols:
        ...
        keep.append(c)
    if not keep:
        raise ValueError("student feature_cols is empty after filtering")
    return keep


def ensure_teacher_columns(teacher_df: pd.DataFrame) -> pd.DataFrame:
    out = teacher_df.copy()
    required = ["row_id", "teacher_prob_binary"]
    for c in required:
        if c not in out.columns:
            raise ValueError(f"Teacher prediction file missing required column: {c}")
    for c in ["teacher_attack_prob_1", "teacher_attack_prob_2", "teacher_attack_prob_3"]:
        if c not in out.columns:
            out[c] = 0.0
    probs = out[["teacher_attack_prob_1", "teacher_attack_prob_2", "teacher_attack_prob_3"]].astype(float).values
    ...
    out[["teacher_attack_prob_1", "teacher_attack_prob_2", "teacher_attack_prob_3"]] = probs.astype(np.float32)
    return out
...
class RiskCascadeDataset(Dataset):
    def __init__(self, df: pd.DataFrame, feature_cols: List[str], scaler: StandardScaler, teacher_df: pd.DataFrame):
        df = df.copy()
        teacher_df = ensure_teacher_columns(teacher_df)
        merge_cols = [
        ...
        if miss > 0:
            raise RuntimeError(f"Teacher predictions failed to align with split rows, missing {int(miss)} rows")
        ...
        # abnormal: original 1/2/3 -> internal 0/1/2; normal uses placeholder 0 and is masked out.
        ...
        self.split_group_id = df["split_group_id"].astype(str).values if "split_group_id" in df.columns else np.array([""] * len(df))
...
def masked_weighted_ce(logits: torch.Tensor, target: torch.Tensor, mask: torch.Tensor, weight: torch.Tensor) -> torch.Tensor:
    ...
    if torch.any((target < 0) | (target >= n_classes)):
        ...
        raise ValueError(f"attack target out of range, expected in [0, {n_classes-1}], but found {bad}")
...
def masked_attack_kd(student_logits: torch.Tensor, teacher_probs: torch.Tensor, mask: torch.Tensor, temperature: float) -> torch.Tensor:
    ...
    if torch.isnan(q).any() or torch.isinf(q).any():
        raise ValueError("teacher attack probs contain NaN/Inf")
...
def eval_dataset(model, loader, ds: RiskCascadeDataset, device, bin_threshold: float):
    ...
    risk_metrics["confusion_matrix"] = confusion_matrix(ds.y_risk.numpy().astype(np.int64), pred_risk, labels=RISK_CLASSES).tolist()
...
def main() -> None:
    parser = argparse.ArgumentParser(description="ECU-IoFT student cascade compatible with attack/run/time-level splits")
    parser.add_argument("--data-dir", type=str, default="Dataset/ecu_attack_risk_windows_time_level")
    parser.add_argument("--teacher-dir", type=str, default="output_time/teacher_risk_cascade_v1")
    parser.add_argument("--output-dir", type=str, default="output_time/student_risk_cascade_v1")
    ...
    teacher_train = ensure_teacher_columns(pd.read_csv(Path(args.teacher_dir) / "teacher_risk_preds_train.csv"))
    teacher_val = ensure_teacher_columns(pd.read_csv(Path(args.teacher_dir) / "teacher_risk_preds_val.csv"))
    teacher_test = ensure_teacher_columns(pd.read_csv(Path(args.teacher_dir) / "teacher_risk_preds_test.csv"))
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        history.append({
            ...
            **{f"train_binary_{k}": v for k, v in train_bin_m.items() if k not in {"support", "labels"}},
            **{f"train_risk_{k}": v for k, v in train_risk_m.items() if k not in {"confusion_matrix", "support", "labels", "precision_per_class", "recall_per_class", "f1_per_class"}},
            **{f"val_binary_{k}": v for k, v in val_bin_m.items() if k not in {"support", "labels"}},
            **{f"val_risk_{k}": v for k, v in val_risk_m.items() if k not in {"confusion_matrix", "support", "labels", "precision_per_class", "recall_per_class", "f1_per_class"}},
            "score": float(score),
        })
        ...
        print(
            ...
            f"val_high_recall={val_risk_m['high_risk_recall']:.4f} lr={optimizer.param_groups[0]['lr']:.2e}"
        )
    ...
    if best_state is None:
        raise RuntimeError("No valid best_state obtained")
    ...
    summary = {
        "task_design": {
            "student": "shared backbone + binary head + fixed 3-class attack-type head",
    ...
    for split_name, loader, ds in [("train", train_eval_loader, train_eval_ds), ("val", val_loader, val_ds), ("test", test_loader, test_ds)]:
        ...
        out = pd.DataFrame({
            ...
            "split_group_id": ds.split_group_id,
