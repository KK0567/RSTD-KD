...
ALWAYS_DROP_EXACT = {
    ...
    # Compatible with audit fields from run/time-level data construction scripts
...
class ConstantAttackClassifier:
    ...
    def __init__(self, constant_class: int):
        if int(constant_class) not in ATTACK_CLASSES:
            raise ValueError(f"constant_class must be in {ATTACK_CLASSES}, got {constant_class}")
...
def load_prepared_splits(data_dir: str) -> Tuple[pd.DataFrame, pd.DataFrame, pd.DataFrame]:
    ...
    if not all(p.exists() for p in paths.values()):
        raise FileNotFoundError(f"{data_dir} is missing train/val/test_windows.csv")
...
def build_binary_xyw(df: pd.DataFrame, feature_cols: List[str]):
    ...
    if len(np.unique(y)) < 2:
        raise ValueError(
            "Teacher binary training set must contain both normal and attack."
            f"Current binary_label distribution={pd.Series(y).value_counts().sort_index().to_dict()}."
            "Reconstruct the run/time-level split, or do not use this split for training."
        )
...
def build_attack_xyw(df: pd.DataFrame, feature_cols: List[str]):
    ...
    if sub.empty:
        raise ValueError("Teacher attack-type model training set has no attack samples; cannot train risk cascade model.")
...
def train_attack_model(train_df: pd.DataFrame, feature_cols: List[str], args):
    ...
    if len(unique) == 1:
        print(f"[TeacherRisk] Warning: train split has only one attack type {unique[0]}, using ConstantAttackClassifier.")
...
def predict_attack_proba_full(attack_model, X: np.ndarray) -> np.ndarray:
    ...
    if np.any(bad):
        # Fallback to uniform distribution if the model returns no known attack type
        # probabilities, to avoid all-zero probabilities in student KD.
