...
"""
ECU-IoFT window 数据构建与模型兼容划分脚本（支持 attack-stratified / run-level / time-level 划分）。
...
- TELLO API Exploit 窗口数很少，attack-stratified 下采用 rare 类逐窗口分配；run/time-level 下应在论文中如实报告类别缺失或样本稀疏问题。
"""
...
def select_window_attack_label(labels: pd.Series, label_rule: str = "dominant") -> str:
    ...
    dominant:
        多数投票。适合普通分类，但会吞掉短持续攻击。
    any_attack:
        只要窗口内存在攻击，就选择窗口内攻击包的主导攻击类型。
    highest_risk:
        按风险等级选择窗口标签。适合风险预警：No Attack < Deauth < WPA2/TELLO。
        同风险内再按 ATTACK_PRIORITY 选择更具体/更高优先级攻击。
    """
    ...
    if unknown:
        raise ValueError(f"未知 Type of Attack: {unknown[:10]}")
...
def build_features_one_window(sub: pd.DataFrame, window_id: int, label_rule: str = "dominant") -> Dict:
    ...
    feat = {
        ...
        # run/time-level 划分需要这些窗口级 run 审计字段。
        # window_run_id 来自原始包级 _raw_run_id 的主导值；
        # window_run_mixed=1 表示该窗口跨越了多个 run，应在严格 run-level 下避免。
...
def build_raw_run_ids(raw: pd.DataFrame, run_col: str = "", time_gap_sec: float = 60.0) -> pd.DataFrame:
    ...
    优先使用显式 run/session/capture 列；如果没有，则用“Attack Scenario + Type of Attack”的连续片段
    加时间间隔阈值自动切分。这个自动 run 只能作为弱替代，不等价于真实采集轮次。
    """
    out = raw.copy()
...
def build_windows(df: pd.DataFrame, window_size: int, step_size: int, within_run: bool = False, keep_short_runs: bool = False, label_rule: str = "dominant") -> pd.DataFrame:
    ...
    if within_run:
        if "_raw_run_id" not in df.columns:
            raise ValueError("within_run=True 时需要先调用 build_raw_run_ids 生成 _raw_run_id")
    ...
    for part in iterable:
        ...
        if len(part) < window_size:
            if keep_short_runs and len(part) > 0:
                # 严格 run-level 下，短攻击 run 如果直接丢弃，会导致某些风险类全局消失。
                # 这里允许短 run 生成一个变长窗口；特征仍为统计量，模型可通过 window_packet_count 感知窗口长度。
                sub = part
    ...
    if out.empty:
        raise ValueError("窗口为空，请减小 window_size，或关闭 --window-within-run，或检查 run 切分后每个 run 的长度")
    return out
...
def assign_splits_run_level(
    ...
    """
    run-level 划分：同一个 run/session 的所有窗口只能进入同一个 split。
    为兼顾 teacher/student 的训练要求，这里按 run 的主导 y_attack_type 分层后，再以 run 为单位分配。
    """
    assert abs(train_ratio + val_ratio + test_ratio - 1.0) < 1e-8
    if group_col not in win_df.columns:
        raise ValueError(f"run-level 划分需要 {group_col} 列；请先构造 run id")
    ...
    for gid, sub in out.groupby(group_col, sort=False):
        # 一个 run 内如果跨标签，说明原始 run 构造太粗；这里以窗口多数标签作为主导类型。
    ...
    if (out["split"] == "").any():
        raise RuntimeError("run-level 划分存在未分配 split 的窗口")
    return out
...
def assign_splits_time_level(
    ...
    """
    time-level 划分：严格按窗口起始时间/包序号排序，前段训练，中段验证，后段测试。
    该模式不做类别回填；如果某一风险类只出现在未来测试段，训练集就会缺类，脚本会如实报错。
    """
    assert abs(train_ratio + val_ratio + test_ratio - 1.0) < 1e-8
    ...
    if n_test <= 0:
        raise ValueError("time-level 划分后 test 为空，请调整 train/val/test 比例")
...
def assert_model_compatible_splits(
    ...
    注意：严格 run-level / time-level 下，不能再强制 train/val/test 都包含完整攻击类型，
    否则会破坏“同一 run 不跨集合”或“按时间外推”的验证意义。

    - 使用旧版 teacher/student 时，train 至少需要攻击类型 1/2/3，否则 teacher_attack_prob_1/2/3 会报错。
    - 使用 run_time_compatible 版 teacher/student 时，可以允许 train 缺少某些攻击类型，
      teacher 会固定补齐 teacher_attack_prob_1/2/3。
    """
    ...
    for split in ["train", "val", "test"]:
        ...
        if require_all_risks_each_split and miss_risk:
            ...
            warnings.append(f"{split} 缺少风险类别 y_risk={miss_risk}，当前={sorted(risk_present)}")
        ...
        if require_all_attack_types_each_split and miss_attack:
            ...
            warnings.append(f"{split} 缺少攻击类型 y_attack_type={miss_attack}，当前={sorted(attack_present)}")
    ...
    if require_train_attack_types_for_old_teacher and miss_train_attack:
        problems.append(f"train 缺少旧版 teacher 攻击模型需要的攻击类型 {miss_train_attack}")
    elif miss_train_attack:
        warnings.append(
            f"train 缺少攻击类型 {miss_train_attack}；如使用 run_time_compatible teacher/student 可继续，"
            f"如使用旧版 teacher/student 会报错"
        )

    if warnings:
        print("[SplitCheck][WARNING] 严格划分导致部分类别缺失：")
        for w in warnings:
            print(f"  - {w}")
...
def main() -> None:
    ...
    parser.add_argument(
        ...
        help="窗口标签规则：dominant=多数投票；any_attack=窗口内有攻击则按攻击主导类；highest_risk=按最高风险标注，推荐用于风险预警。",
    )
    parser.add_argument(
        "--keep-short-runs",
        action="store_true", default=True,
        help="run/time-level 下保留长度小于 window-size 的短 run，生成一个变长窗口，避免短攻击片段被全部丢弃。",
    )
    ...
    parser.add_argument(
        ...
        help="attack_stratified=原始按攻击类型分层；run_level=按采集轮次/连续片段分组；time_level=按时间前后切分。",
    )
    parser.add_argument("--run-col", type=str, default="", help="原始CSV中的真实 run/session/capture 列名；没有则留空自动构造。")
    parser.add_argument("--run-gap-sec", type=float, default=60.0, help="没有真实 run 列时，按时间间隔切分连续片段的阈值。")
    parser.add_argument(
        "--window-within-run",
        action="store_true",
        help="窗口只在同一个 run 内生成，避免窗口跨越两个采集片段。run/time-level 建议开启；选择 run_level 时会自动开启。",
    )
    parser.add_argument(
        "--allow-missing-attack-type-in-val-test",
        action="store_true", default=True,
        help="不强制 val/test 同时包含 0/1/2/3。run/time-level 通常需要开启或保持默认自动放宽。",
    )
    parser.add_argument(
        "--relaxed-split-check",
        action="store_true", default=True,
        help="不强制每个 split 都包含 y_risk=0/1/2；用于严格 time-level 导致某些风险类自然缺失的情况。",
    )
    parser.add_argument(
        "--require-all-attack-types-each-split",
        action="store_true",
        help="强制每个 split 都包含 0/1/2/3。严格 run/time-level 下通常不建议开启。",
    )
    parser.add_argument(
        "--use-old-teacher",
        action="store_true",
        help="如果仍使用旧版 teacher/student，则强制训练集包含攻击类型1/2/3；使用 run_time_compatible 版时不要开启。",
    )
    ...
    # run-level 必须避免窗口跨 run；time-level 也建议避免跨片段窗口，以降低标签污染。
    ...
    if args.split_strategy == "attack_stratified":
        ...
        raise ValueError(f"未知 split_strategy: {args.split_strategy}")
    ...
    manifest = {
        ...
        "notes": [
            ...
            "attack_stratified 是模型兼容性最强的常规划分，但不是严格 time-level 验证。",
            "run_level 保证同一 run/window_run_id 不跨 train/val/test；若 run 是自动构造的连续片段，应在论文中说明其弱替代属性。",
            "time_level 采用过去训练、未来验证/测试，不做类别回填；若类别自然缺失，应如实报告。",
            "y_risk=1 在现有 teacher/student 中命名为 medium；如论文写低风险，请将其解释为中低风险/较低危害攻击。",
            "TELLO API Exploit 窗口数极少，run/time-level 下不宜夸大该类型泛化能力。",
            "若 window_attack_type_counts 缺少 y_attack_type=1，请优先尝试 --window-label-rule highest_risk 与 --keep-short-runs。",
        ],
    }
